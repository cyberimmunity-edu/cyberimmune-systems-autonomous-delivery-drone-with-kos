...
if [[ $* == *"--with-obstacles"* ]]
	then
		tmux send-keys -t flight_controller "cd ardupilot; ./run_in_terminal_window.sh ArduRover sitl/ardurover_obstacles -S --model rover-skid --speedup 1 --slave 0 --serial5=tcp:5765:wait --serial6=tcp:5766:wait --serial7=tcp:5767:wait --defaults rover.parm --sim-address=127.0.0.1 --home=53.1019446,107.3774394,846.22,0 -I0" Enter
	else
		tmux send-keys -t flight_controller "cd ardupilot; ./run_in_terminal_window.sh ArduRover sitl/ardurover -S --model rover-skid --speedup 1 --slave 0 --serial5=tcp:5765:wait --serial6=tcp:5766:wait --serial7=tcp:5767:wait --defaults rover.parm --sim-address=127.0.0.1 --home=53.1019446,107.3774394,846.22,0 -I0" Enter
fi
