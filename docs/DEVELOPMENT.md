...
1. Сборка и запуск осуществляется при помощи скрипта `run.sh`
    ...
    1. Если доступ к серверу СУПА отсутствует, то можно запустить симулятор без него. В этом случае все обращения к серверу СУПА будут считаться успешными. Также стоит учесть, что в этом случае в KOS не будет загружена миссия
...
1. Произвести Arm повторно; перевести дрон в режим 'Auto' для начала исполнения миссии
