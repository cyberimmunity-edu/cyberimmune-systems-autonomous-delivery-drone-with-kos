...
При запуске `make online` задание подтверждается через СУПА, а значит в коде нужно учитывать получил ли контроллер задание, а иначе логика может отработать до его получения и ничего не вывести на экран.
...
Исходя из архитектуры предполагается, что в запуске машинки участвуют как минимум две роли: оператор машинки и диспетчер системы управления парком автомобилей (СУПА).
...
Для экспериментальных целей или отладки вы можете менять любую часть проекта, но для соревнований реализация учитывается только внутри `kos/flight_controller`.
...
## Вопрос 25: Последовательность действий для запуска без СУПА (offline):
  ...
  4. Загрузили задание в APM Planner 2 (Flight plan -> Edit waypoints -> Load WP -> Write)
...
## Вопрос 26: Последовательность действий для запуска с СУПА (online):
  ...
  3. Дождались когда всё загрузилось и подключилось
  4. Загрузили задание в APM Planner 2 (Flight plan -> Edit waypoints -> Load WP -> Write)
  5. Зашли в СУПА и посмотрели подключился ли дрон и какой у него ID
  6. Загрузили задание в СУПА, используя Mission Sender
  7. В СУПА подтвердили миссию и разрешили взлёт
  8. В APM Planner 2 ткнули ARM (запросили разрешение)
  9. В СУПА разрешили запуск двигателей
...
Однако, намного удобнее загрузить задание из файла в APM Planner 2 и все поля получат текстовую расшифровку.
...
## Вопрос 32: Может ли квадрокоптер в течение задания совершить посадку и повторный взлет?
...
## Вопрос 39: Как изменить положение точки HOME у цифрового двойника?
...
В сценарии, предполагаемом соревнованиями, для автопилота эта точка не влияет ни на что. Однако именно она передается в модуль безопасности передается и отображается при вызове printMission(), поэтому предполагается, что ей нужно совпадать с настоящим положением дома.
