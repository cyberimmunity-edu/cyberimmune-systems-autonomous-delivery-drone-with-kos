...
При запуске `make online` полётное задание подтверждается через СУПА, а значит в коде нужно учитывать получил ли полётный контроллер задание иначе логика может отработать до его получения и ничего не вывести на экран.
...
Исходя из архитектуры предполагается, что в запуске квадрокоптера участвуют как минимум две роли: оператор квадрокоптера и диспетчер системы управления парком автомобилей (СУПА).
...
- регистрирует полётное задание в СУПА для конкретного квадрокоптера по его Drone ID используя Mission Sender,
- запускает квадрокоптер получив подтверждение от СУПА.
...
## Вопрос 26: Последовательность действий для запуска с СУПА (online):
  ...
  3. Дождались когда всё загрузилось и подключилось
  4. Загрузили полётное задание в APM Planner 2 (Flight plan -> Edit waypoints -> Load WP -> Write)
  5. Зашли в СУПА и посмотрели подключился ли квадрокоптер и какой у него ID
  6. Загрузили полётное задание в СУПА используя Mission Sender
