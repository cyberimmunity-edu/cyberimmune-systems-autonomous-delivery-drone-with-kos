...
int main(int argc, char* argv[]) {
    ...
    fprintf(stderr, "Info: generating RSA keys and sharing it with the server\n");
    ...
    fprintf(stderr, "Info: authenticating on the server\n");
    while (!sendRequest("auth", serverResponse)) {
        fprintf(stderr, "Warning: failed to authenticate on the server. Repeating in %d s...\n", requestRepeatDelay);
        sleep(requestRepeatDelay);
    };
    fprintf(stderr, "Info: authentication message is completed\n");

    fprintf(stderr, "Info: requesting mission from the server\n");
    while (!requestMission()) {
        fprintf(stderr, "Warning: failed to get mission from the server. Repeating in %d s...\n", requestRepeatDelay);
        sleep(requestRepeatDelay);
    };
    fprintf(stderr, "Info: correct mission is received from the server\n");
    printMission();

    fprintf(stderr, "Info: ready to arm\n");
    while (true) {
        while (!waitForCommand()) {
            fprintf(stderr, "Warning: some problem happened during waiting for arm request from autopilot\n");
            sleep(1);
        }
        fprintf(stderr, "Info: arm request from autopilot is received. Sending request to the server\n");
        startBlinking();
        ...
        if (strstr(serverResponse, "$Arm: 0#") != NULL) {
            fprintf(stderr, "Info: response on arm request from the server is received: arm is permitted\n");
            if (!setLight(true))
            ...
            fprintf(stderr, "Info: response on arm request from the server is received: arm is prohibited\n");
            if (!setLight(false))
    ...
    while (true) {
        if (sendRequest("fly_accept", serverResponse)) {
            if (strstr(serverResponse, "$Arm: 0#") != NULL) {
                ...
                fprintf(stderr, "Info: a request to stop the flight was received from the server\n");
                if (!setLight(false))
                    fprintf(stderr, "Warning: failed to turn off the light\n");
    ...
    while (true) {
        fprintf(stderr, "Info: finishing flight...\n");
        sleep(5);
    }
