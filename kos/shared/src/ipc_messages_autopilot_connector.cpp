...
int setMission(uint8_t* mission, uint32_t missionSize) {
    ...
    nk_uint8_t *msg = nk_arena_alloc(nk_uint8_t, &reqArena, (nk_ptr_t*)(&(req.mission)), missionSize);
    if ((msg == NULL) || (missionSize > AutopilotConnectorInterface_SetMission_req_arena_size))
        return 0;
    memcpy(msg, mission, missionSize);

    return ((AutopilotConnectorInterface_SetMission(&proxy.base, &req, NULL, &res, NULL) == rcOk) && res.success);
}